    ...
    run: async (bot, channel) => {
        ...
        if (data.logs.channel.actived) {
            const embed = new Discord.MessageEmbed()
                .setAuthor({name: `Channel deleted | ${channel.guild.name}`, iconURL: channel.guild.iconURL()})
                .setColor("07AE46")
                ...
                .addField(
                    ...
                    `${Discord.Formatters.codeBlock(
                        "",
                        `<t:${Math.floor(channel.createdTimestamp / 1000)}:D> <t:${Math.floor(channel.createdTimestamp / 1000)}:R>`
                    )}`,
                    true
                )
                .addField(
